...
@Component({
  selector: 'app-petitioners-draft-list',
  templateUrl: './petitioners-draft-list.component.html',
  styleUrls: ['./petitioners-draft-list.component.scss']
})
export class PetitionersDraftListComponent implements OnInit, OnDestroy {
    ...
    this.dtOptions = this.datatablesService.buildOptions({
      ajax: (params: any, callback) => {
        ...
        this.subscription = this.petitionService
          .getPetitionsList(pageParams, sortParams, search, PetitionsGroups.Draft)
          .subscribe(response => {
      ...
      columns: [{
        ...
        orderable: false,
        render: (data: boolean) => `
        <button class="btn btn-primary btn-sm" ${this.disable({disableStatus: [1, 3]})}>Publicar</button>
        <button class="btn text-danger" ${this.disable({disableStatus: [1]})}>Eliminar</button>`
      }],
      drawCallback: () => {
  ...
  private goToDetail(data: any) {
    this.router.navigate([`petition/create/${data.id}`]);
  }
